...
for file_num in range(0,10,1):
	if frame_or_no == "f":
		infile1 = open('./input_corpora/'+str(file_num)+'chunck_train_ff_adultNEW.txt','r')
		infile2 = open('./input_corpora/'+str(file_num)+'chunck_test_ff_adultNEW.txt','r')#
		readinfile1 = infile1.readlines()
		readinfile2 = infile2.readlines()
		infile1.close()
		infile2.close()
	elif frame_or_no == "c":
		infile1 = open('./input_corpora/'+str(file_num)+'chunck_train_gold_adultNEW.txt','r')
		infile2 = open('./input_corpora/'+str(file_num)+'chunck_test_gold_adultNEW.txt','r')#
		readinfile1 = infile1.readlines()
		readinfile2 = infile2.readlines()
		infile1.close()
		infile2.close()
	...
	if frame_or_no == "f":
		...
		for ff_thing in ff_FRAMES.keys():
			print("ff frame!", ff_thing)
#~*~*~*~*~*~*~~*~*
#*~*~*~*~*~*~*~*~okay this is to get the categories from an output file 
	elif frame_or_no == "c":
		infile_cats = open('NEW_cat_adult_eng.txt', "r")
	...
	def first_element_cat(test, g,r):#~*~*~*~*~**~**~fix for utterances two words or less!!!
		...
		final_first = 0
		for sentence in r:
			sent_cat = sentence[1].split("**")
			if test[1] in checklist:
				if first_word == sent_cat[0] and category == sent_cat[1]:#start-word(['category
					count +=1
		utt_prob_start = (count+.5)/(len(r)+.5)#.5 for smoothing
		#trying other smoothing
		if category in g and first_word in g[category]:
			...
			for num in range(1,(len(test)-2),1):
				...
				for sentence in r:
					for x in range(1,len(sentence)-2,1):
						word_plus_cat_sent1 = sentence[x].split("**")
						word_plus_cat_sent2 = sentence[x+1].split("**")
						#~*~*~*~*~~*different options for if you see word in training
						if test[num] in checklist and test[num+1] in checklist:
							if word_plus_cat_sent1[1] == word_plus_cat[1] and word_plus_nextcat[1] == word_plus_cat_sent2[1]:
								new_count += 1
					for pos in range(1,len(sentence)-1,1):
						any_word_cat = sentence[pos].split("**")
						if any_word_cat[1] == category:
							count += 1
				cat_instances = count
				following_cat = new_count
				one_word_prob = (new_count+.5)/(count+.5)#.5 for smoothing
	...
	def last_element_cat(test,r):
		...
		last_prob = 0
		for sentence in r:
			word_plus_cat_end = sentence[-2].split("**")
			for x in range(1,len(sentence)-1,1):
				word_plus_cat_end2 = sentence[x].split("**")
				if word_plus_cat_end2[1] == category:
					count +=1
			if word_plus_cat_end[1] == category:
				ends +=1
		last_prob = (ends+.5)/(count+.5)
		print("last ok!")
		return log10(last_prob)
	...
	outfile = open(str(file_num)+"perplex_w_freq_ff_adult.txt","w")
	outfile2 = open(str(file_num)+"perplexity_gold_adult.txt", "w")
