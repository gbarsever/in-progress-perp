...
class BHMMI:
    ...
    def sample(self, num_iters=10, anneal=False):
        ...
        for iteration in range(num_iters):
            ...
            for i in range(self.N): #GB: going through all the words in the corpus
                if self.X[i] > 0: # not boundary (don't resample boundaries)
                    self.decrement(i) #GB: figure out what this is
                    new_y = self.sample_y(i, temp) #GB: this is sampling a new tag?
                    self.Y[i] = new_y #tag corresponding to tag in array is changed
                    self.increment(i) #GB: same as decrement
    ...
    def sample_y(self, i, temp=1):
        ...
        probs = np.zeros(self.K, float) #GB: creates array with #types of tags zeros
        for y in range(1, self.K): # GB: y is a tag type <---- could adding the extra category be here? and then if it goes to the extra, add it to matrix and self.K?
        ...
        new_y = np.argmax(np.random.multinomial(1, probs)) #GB: this is where the sampling actually happens
        #print i, probs, new_y
        return new_y #GB: new tag for i?
